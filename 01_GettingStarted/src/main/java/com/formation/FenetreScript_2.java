...
public class FenetreScript_2 {
	...
	private static void activityLesson02AutomationScript() {
		...
		try {
			...
			if (driver.manage().window().getSize().getHeight() >= 500 && driver.manage().window().getSize().getWidth() >= 300) {
			...
			System.out.println(String.format("Current window size is %s height x %s width", driver.manage().window().getSize().getHeight(), driver.manage().window().getSize().getWidth()));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			driver.quit();
